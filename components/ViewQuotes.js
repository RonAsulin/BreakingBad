...
const ViewQuotes = (props) => {
  return (
    ...
    <View style={myStyles.name_container}>
      <Text style={myStyles.name}>{props.quotes.quote }</Text>
      <Text style={{color:'#00b4d8',fontSize:18}}>{props.quotes.air_date}</Text>
      <View style ={myStyles.line}></View>
      <Text>{props.quotes.author}</Text>
      <Text style={myStyles.occupation}>{props.quotes.series}</Text>
      </View>
